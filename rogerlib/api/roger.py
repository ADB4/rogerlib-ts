...
from rogerlib import limiter

@rogerlib.app.route('/api/v1/cntct/', methods=['GET'])
def get_cntct():
...
@rogerlib.app.route('/api/v1/cntct/deets/', methods=['GET'])
def get_deets():
    deets = fetch_deets()
    context = {
        'deets': deets
...
@rogerlib.app.route('/api/v1/images/items/all/', methods=['GET'])
def get_all_item_images():
    context = {}
    return flask.jsonify(**context)

@rogerlib.app.route('/api/v1/items/<itemcode>/', methods=['GET'])
...
@rogerlib.app.route('/api/v1/items/all/', methods=['GET'])
def get_all_items():
    """Return all items"""
    ct = datetime.datetime.now()
...
@rogerlib.app.route('/api/v1/categories/all/', methods=['GET'])
def get_categories():
    ct = datetime.datetime.now()
...
@rogerlib.app.route('/api/v1/quotes/random/', methods=['GET'])
def get_quotes():
    context = fetch_quote_s3()
    return flask.jsonify(**context)

@rogerlib.app.route('/api/v1/downloads/<itemcode>', methods=['GET'])
